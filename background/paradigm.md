...
编程范式（Programming Paradigm）是指导程序构建的某种“思想”或者“方法论”，它直接影响到代码的结构和风格。每一门编程语言的设计都遵循了一种或者多种编程范式，无论它有没有明确说明。

到今天，编程范式已经发展出一个[很多元的体系](https://en.wikipedia.org/wiki/Programming_paradigm)，每个领域的开发者都不可能也不需要熟知所有语言和范式。了解编程范式的目的不仅仅是了解它们各自代表的含义，更重要的是认识到自己所处的位置和局限性：

* 语言的背后有它的理论体系，不是没有章法的。
* 编写程序的方式可以有很多种，而不是只有自己熟知的那几种。

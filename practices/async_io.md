...
![Socket Communication](pic/tcpip_socket.png)
...
![BSD Socket APIs](pic/bsd_socket_api.png)
...
而使用Blocking IO时，主线程在接受一个连接后，会阻塞在调用处等待数据就绪，期间无法处理其它连接请求（阻塞IO实现的socket server参见*code/socket_server_blocking.cpp*）。解决该问题可以借助多线程/多进程（One Thread per Connection）。主线程在accept之后创建一个线程执行read和write操作，然后马上返回accept处，等待新的连接请求。这个解决方案可以处理并发连接，但是线程作为操作系统的资源，创建、销毁、调度都是有开销的，不可能不停地创建。
...
想要提高并发处理能力，要解决的根本问题是**高效地等待IO就绪**。这涉及整个系统栈，包括硬盘/网卡、驱动、操作系统和应用层代码。任务完成这个“信号”到底是由硬盘/网卡等硬件发出的，只有它经过的每个环节都高效地处理信号，才能达到最终整体高效的效果。做到这点，需要使用内核实现的IO多路复用(I/O multiplexing)机制。这一机制将等待的工作移到了内核当中，内核代码和驱动通过更高效的实现减少在等待上耗费的无用功。
...
* 同步和异步用于描述更广泛的编程模式，不限于IO。关注点是**完成**某一任务（Task，例如读取128字节完成）的方式，是调用返回就代表完成，还是立马返回完成后通过回调等方式通知。

在这个意义上讲，执行Blocking和Non-blocking IO系统调用是同步操作。基于IO多路复用的select/epoll调用也都是同步的，它们虽然会过滤出有IO事件发生的socket，但是不会执行数据读写操作。真正的异步IO通常指的是用户提交IO请求和数据缓冲区，数据读写完成后再通知用户。

通过封装IO多路复用机制，很容易转换出回调形式的异步接口。遵循这种思路实现的跨平台封装有：
...
内核的AIO模块试图把IO操作以统一的异步接口提供给开发者。select/epoll等多路复用APIs在AIO中不再存在，开发者将fd和读写缓冲区传入AIO APIs，内核在IO任务完成之后通知调用方。

Windows上的IOCP是一个真正意义上高效的AIO实现，内核维护了一个消息队列存放已经完成的任务，这个队列被称为“完成端口”。而Linux内核[AIO](https://github.com/littledan/linux-aio)模块的开发持续了很长时间也没有成熟，似乎陷入了泥藻。目前Linux上常见的AIO应用场景是读写磁盘文件，在网络socket上的稳定应用十分罕见。这导致Linux AIO甚至被理解为异步IO针对磁盘文件的特化方案。

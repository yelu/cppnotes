...
促使异步模式被采用的动机通常是有些操作执行速度太慢，调用方无法接受一直等待下去，不得不放弃等待。调用方执行async函数时会注册一个回调函数(callback)，然后立即返回，在异步操作完成后，callback函数会被执行。本质上，异步是一个连贯、完整的执行流程，被耗时操作切割成多个片段，断断续续地执行。由于涉及到上下文状态的保存和恢复，异步执行比同步顺序执行要复杂得多。

```cpp
void async_call(arg1, arg2, ..., callback);
```

常见的耗时任务分为两类：CPU密集型(CPU-bound)和IO密集型(IO-bound)。尽管二者对外表现类似，都是执行时间长，从对系统资源的有效利用角度看，二者存在很大差异。不同的特征导致异步执行的实现方式也有所不同。
...
在用户量很大的高并发场景下，改进IO-bound任务的执行效率可以极大提升系统的吞吐量。但是这种改进涉及整个系统栈，包括硬盘/网卡、驱动、操作系统和应用层代码。任务完成这个“信号”到底是由硬盘/网卡等硬件发出的，只有它经过的每个环节都高效地处理信号，才能达到最终整体高效的效果。IO操作作为这类任务的代表，在很多工具库或者文章里面，提到异步甚至指的就是异步IO。为了不打断本节的连贯性，异步IO将在下一节单独介绍。

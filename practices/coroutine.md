...
协程和进程、线程一样，逻辑上看都是流程并发的某种机制。进程和线程(参见附录II)是开发者耳熟能详的概念，OS对多个进程进行抢占式调度，如果进程内部还需要多个并发流程就使用线程。还需要协程干什么呢？

协程所作的事情，可以由独立的线程实现。但是线程的抢占式调度机制，提供不了协程的效率。操作系统的调度要兼顾公平，它也无法了解代码逻辑所蕴含的信息，因此在切换时机的选择上做不到协程那么准确：协程本质上是一种协作式的调度机制，它只在有必要切换的时候切换。

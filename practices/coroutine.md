...
在用户量很大的高并发场景下，改进IO-bound任务的执行效率可以极大提升系统的吞吐量。但是这种改进的必须涉及整个系统栈，包括硬盘/网卡、驱动、操作系统和应用层代码。任务完成这个“信号”到底是由硬盘/网卡等硬件发出的，只有它经过的每个环节都高效地处理信号，才能达到最终整体高效的效果。IO操作作为这类任务的代表，在很多工具库或者文章里面，提到异步甚至指的就是异步IO。为了不打断本节的连贯性，异步IO的技术演进将在下一节单独介绍。
...
协程和进程、线程一样，逻辑上看都是流程并发的某种机制。协程所作的事情，看起来也可以由进程或线程实现，为什么还需要协程呢？
...
在Coroutine进入C++标准之前，[boost::context](http://www.boost.org/doc/libs/1_57_0/libs/context/doc/html/index.html)就对实现协程提供了支持，实现方式是保存多条执行路径的栈和寄存器。随着Coroutine正式进入C++ 20，微软提出了一个无栈实现。boost::asio也已经完全支持协程，它提供co_spawn函数来启动一个协程，每个异步函数都有协程重载版本，使用起来就像在写同步程序一样。基于协程在C++ 20的落地，asio会在接下来的时间里继续改进对协程的支持，以更加合理、优雅、高效的方式准备加入到C++ 23。

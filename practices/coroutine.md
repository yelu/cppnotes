...
促使异步模式被采用的动机通常是有些操作执行速度太慢，调用方无法接受一直等待下去，不得不放弃等待。调用方执行async函数时会注册一个回调函数(callback)，然后立即返回，在异步操作完成后，callback函数会被执行。

```cpp
void async_call(arg1, arg2, ..., callback);
```

异步执行耗时任务的常见模式是，维持一个任务队列(Task Queue)和后台线程池(Worker Thread Pool)。Worker Thread不停地从队列中拿任务执行，执行完成调用注册的callback或者其它机制通知调用方。通过将任务交给线程池执行，调用方不会再被阻塞在调用处。

```cpp
// 回调函数
```

常见的耗时任务分为两类：CPU密集型(CPU-bound)和IO密集型(IO-bound)。尽管二者对外表现类似，都是执行时间长，从对系统资源的有效利用角度看，二者存在很大差异。不同的特征导致异步的实现方式也有所不同。

高效执行IO-bound的任务比CPU-bound的任务要更加复杂。采用以上多线程模型，将IO操作放进单独的Worker线程去处理，功能上是没有问题的。但这仅仅是将等待的工作从调用方线程转移到了另一个线程中去，资源利用率方面没有改进。IO-bound的任务是由于磁盘和网络等设备相对CPU过慢，让CPU等待是浪费资源，不如让这些设备在数据就绪时主动通知CPU，CPU在这段时间内可以用于执行其它运算。IO操作作为异步任务的代表，在很多工具库或者文章里面，提到异步甚至指的就是异步IO。为了不打断本节的连贯性，异步IO将在下一节单独介绍。

尽管CPU-bound和IO-bound的任务有所不同，编程语言也还是应该考虑为开发者提供类似的接口、统一的体验。但是这并不是现实，历史上C++对异步的态度一直都是放任的，缺乏统一的标准，导致开发者具体问题具体对待，长期处于分化状态。例如，处理网络IO时，有人喜欢libevent/asio/mudoo等基于callback的工具库，也有人喜欢直接操作BSD APIs和epoll；处理耗时的CPU运算，有人简单启动一个线程处理，也有人用复杂的状态机去记录状态跳转，是否支持中途取消运算(Cancellation)和超时(Timeout)也各有各的考虑。反倒是C#在异步方面的设计最为干净和考究，它最先在语言层面提供了稳定的async/await/task APIs，开辟了一条道路引领大家思考编程语言应该如何在语法层面简化异步编程。
...
相比简单的串行同步执行模式，异步回调改进了程序的响应模式，提高了系统的处理能力，但代码的复杂度也增加不少。异步是一个连贯、完整的执行流程，被耗时操作切割成多个片段，断断续续地执行。由于涉及到上下文状态的保存和恢复，回调模式很容易进入一个灾难状态，就是所谓的callback hell。

...
常见的耗时任务分为两类：CPU密集型(CPU-bound)和IO密集型(IO-bound)。在对系统资源的有效利用方面，二者有所不同，导致异步的底层实现方式也需要有所区别。对于CPU-bound任务，线程池模型是合适的。而对于IO-bound的任务，将IO操作放进单独的Worker线程，功能上是没有问题的，但这仅仅是将等待的工作从调用方线程转移到了另一个线程中去，资源利用率方面没有改进。IO-bound的任务是由于磁盘和网络等设备相对CPU过慢，让CPU等待是浪费资源，不如让这些设备在数据就绪时主动通知CPU，CPU在这段时间内可以用于执行其它运算。IO操作作为异步任务的代表，在很多工具库或者文章里面，提到异步甚至指的就是异步IO。为了不打断本节的连贯性，异步IO将在下一节单独介绍。
